...
- **Monthly_Sheet**: 既存の月別シート（1月〜12月）。A列に日付、B列に金額、C列にカテゴリ、D列にカード払いフラグ（TRUE/FALSE）が格納される
...
- **家計立替**: クレジットカード払いとしてフラグが立った経費の合計額。選択中の期間に連動して Pie_Chart_Area 内に表示される
...
4. THE 家計立替金額 SHALL D列が `TRUE` または `○` の行の金額（B列）を合計した値とする
